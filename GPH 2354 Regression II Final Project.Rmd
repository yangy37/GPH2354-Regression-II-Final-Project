...
hoslem_1;
pander(c("Model 1 AIC" = AIC(model_1), "Model 2 AIC" = AIC(model_1_2), 
         "Model 1 BIC" = BIC(model_1), "Model 2 BIC" = BIC(model_1_2)));
```
...
#Step 1
model_2 <- vglm(health ~ depress + alcage + cigage + age + bp, family = cumulative(parallel=TRUE), data = my_data_2);
summary(model_2);
...
#Step 3
my_data_2_3 <- my_data_2 %>% filter(age == 64);
model_2_3 <- vglm(health ~ depress, family = cumulative(parallel=TRUE), data = my_data_2_3);
summary(model_2_3);
```